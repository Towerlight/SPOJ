...
void AddEdge ( int x, int y ) { g[ ++ sz ].y = y; g[ sz ].frt = pos[ x ]; pos[ x ] = sz; }
void Search ( int x )
{
	siz[ x ] = 1; son[ x ] = 0;
	REP_G ( i, x )
...
void Update ( int x, int l, int r, int loc, int v )
{
	...
	int mid = ( l + r ) / 2;
	Update ( SLC, loc, v ), Update ( SRC, loc, v );
...
int main()
{
	...
	for ( scanf ( "%d", &q0 ); q0; q0 -- )
	{
		...
		Search ( root ), BuildTree ( root, root );
		REP ( i, n - 1 )
		{
